...
* **Backend:** Spring Boot 3.5.3 + Spring Security 6.5.3
...
* **Task Status Tracking:** Todo, In progress, Done status workflow
...
* **Task:** id, title, description, status (enum), userId (foreign key), tagId (foreign key), createdAt, updatedAt
* **Profile:** userId (foreign key), firstName, lastName, email, preferences
* **Tags**: tagId, name, color, createdAt, updatedAt, userId (foreign key)
...
* `GET /api/tags/search` - Search tag to add to task
* `GET /api/tags/count` - Get specific user tag count
...
* **Authenticate** to app
* Update **profile**
* Manage **tasks**, update tasks status
* Manage **tags**, assign tags to tasks

## 12. Non-Functional Requirements

* **Availability:** 99.9% uptime target with graceful degradation
* **Performance:** Support for 100+ concurrent users with sub-200ms response times
...
* **Local Development:** Docker Compose for consistent development environmen
