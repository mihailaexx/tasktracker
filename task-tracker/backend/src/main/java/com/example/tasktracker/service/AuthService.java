...
public class AuthService {
    ...
    public AuthResponse authenticate(LoginRequest loginRequest, HttpServletRequest request) {
        try {
            ...
            return new AuthResponse(false, "Authentication failed", null, null, null);
        } catch (Exception e) {
            return new AuthResponse(false, "Internal server error", null, null, null);
        }
    }
    ...
    public AuthResponse registerUser(String username, String password, String email) {
        try {
            ...
            if (password.length() < 8) {
                return new AuthResponse(false, "Password must be at least 8 characters long", null, null, null);
            }
            ...
            return new AuthResponse(true, "User registered successfully", null, username, "USER");
            ...
            if (errorMessage.contains("username")) {
                ...
                return new AuthResponse(false, "User with these credentials already exists", null, null, null);
            }
        } catch (Exception e) {
            return new AuthResponse(false, "Registration failed" + e.getMessage(), null, null, null);
        }
    }
}